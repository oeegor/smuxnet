...
func (s *ServerV2) GracefulStop() {
	close(s.stop)
	s.wg.Wait()
}

func (s *ServerV2) Serve(handler Handler) chan error {
	s.wg.Add(1)
...
func (s *ServerV2) serve(handler Handler, errs chan error) {
	defer s.wg.Done()
	defer close(errs)
...
func (s *ServerV2) serveSession(conn net.Conn, handler Handler, errs chan error) {
	defer s.wg.Done()
	defer conn.Close()
